...
func (Bar) Scrape(ctx context.Context, sourcePath string) error {
	...
	for scanner.Scan() {
		...
		for _, ingredient := range ingredients.Blocks {
			...
			measurementAsInt, _ := strconv.ParseFloat(measurement.Text, 32)
			...
			if measurement.Text == "" || name.Text == "" || unit.Text == "" {
				...
				measurementAsInt, _ = strconv.ParseFloat(measurement.Decimal, 32)
			}
		...
		for _, instruction := range instructions.Blocks {
			if instruction.ContentType != "callToActionText" {
				...
				cocktail.PushInstruction(strip.StripTags(strings.Replace(text.Html, "\n", "", -1)))
			}
		}
		...
		ingredientsWrapper, _ := searchBlocksForField("ingredientsWrapper", "ingredients", blocks.Blocks)
		units, err := searchBlocksForField("recipeIngredientList", "unitsOfAlcoholPerServe", ingredientsWrapper.Blocks)
		if err == nil {
			unitsToFloat, _ := strconv.ParseFloat(units.Decimal, 32)
			cocktail.Meta.StandardDrinks = math.Round(unitsToFloat*100) / 100
		} else {
			units, err := searchBlocksForField("recipeIngredients", "unitsOfAlcoholPerServe", blocks.Blocks)
			if err == nil {
				unitsToFloat, _ := strconv.ParseFloat(units.Decimal, 32)
				cocktail.Meta.StandardDrinks = math.Round(unitsToFloat*100) / 100
			} else {
				units, _ = searchBlocksForField("ingredientsMethod", "unitsOfAlcoholPerServe", blocks.Blocks)
				unitsToFloat, _ := strconv.ParseFloat(units.Decimal, 32)
				cocktail.Meta.StandardDrinks = math.Round(unitsToFloat*100) / 100
			}
		}
